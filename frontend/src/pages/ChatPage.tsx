...
export default function ChatPage() {
  ...
  const connect = () => {
    ...
    socket.onerror = () => {
      addChatMessage({ id: crypto.randomUUID(), role: 'system', content: 'Connection error - using HTTP fallback', timestamp: Date.now() });
    };
    socket.onclose = () => {
      setConnected(false);
      addChatMessage({ id: crypto.randomUUID(), role: 'system', content: 'Disconnected', timestamp: Date.now() });
    };
    setWs(socket);
  };
  ...
  const sendMessage = async () => {
    ...
    if (ws && ws.readyState === WebSocket.OPEN) {
      ...
      try {
        ...
        addChatMessage({ id: msgId, role: 'assistant', content: full || 'No response', timestamp: Date.now() });
      } catch {
        addChatMessage({ id: crypto.randomUUID(), role: 'system', content: 'Failed to send message', timestamp: Date.now() });
      }
    }
  };
      ...
      <div className="flex-1 bg-slate-800/30 rounded-xl border border-slate-700/50 overflow-y-auto p-4 space-y-3 mb-4">
        {chatMessages.length === 0 && (
          <div className="text-center text-slate-500 py-12 text-sm">
            Send a message or connect via WebSocket to start chatting with OpenClaw
          </div>
        )}
